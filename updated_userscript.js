// ==UserScript==
// @name         Edycja opisów - Local tests
...
// @require https://update.greasyfork.org/scripts/511010/1723392/My%20consts.js
// @grant        none
...
(function () {
  ...
  const TOKEN_API_URL = `${API_BASE_URL}/api/auth/bearer`;
  ...
  const modalPartGenerate = async () => {
    ...
    async function fetchImages() {
      ...
      const apiEndpoint = `${API_BASE_URL}/api/products/images`;
    ...
    async function generateDescriptionWithVisionAPI(imageData) {
      const apiEndpoint = `${API_BASE_URL}/api/translations/generate-description`;
      let preHTML = "<p>";
  ...
  const modalPartRephrase = async () => {
    ...
    async function rephraseDescription(description, options = {}) {
      const apiEndpoint = `${API_BASE_URL}/api/translations/rephrase-description`;
  ...
  const fetchProductData = async () => {
    ...
    const apiEndpoint = `${API_BASE_URL}/api/products/descriptions`;
    ...
    try {
      const response = await fetchWithAuth(apiEndpoint, {
        ...
        body: JSON.stringify({
          ...
          shop_id: shopId,
          lang_id: lang,
          fields: "productName,productLongDescription,productMetaTitle,productMetaDescription,productMetaKeywords"
        }),
      });
      if (!response.ok)
        throw new Error(`HTTP error! status: ${response.status}`);
